...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Business Rule: name must exist, and only one of price or variants
    if (!name) {
    ...
    // Disallow both price AND variants (shouldn't happen with proper UI)
    if (
      (filteredVariants.length > 0 && price) ||
      (filteredVariants.length === 0 && !price)
    ) {
      return NextResponse.json(
        { error: "Provide either a price *or* variants, not both." },
        { status: 400 }
      );
    }
